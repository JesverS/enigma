...
void sauvegarRotorsReflector(RotorsReflector *rotors_reflector, FILE *file){
    if (file == NULL) {
        printf("Erreur lors de l'ouverture du fichier de sauvegarde.\n");
        return;
    }
    fprintf(file, "%s\n", rotors_reflector->reflector);
    for (int i = 0; i < 3; i++)
    {
        fprintf(file, "%d %c %c %c\n", rotors_reflector->l_r[i]->pos, rotors_reflector->l_r[i]->value[0], rotors_reflector->l_r[i]->notch, rotors_reflector->l_r[i]->turnover);
    }
    fclose(file);
}

void chargerRotorsReflector(RotorsReflector *rotors_reflector, FILE *file){
    if (file == NULL) {
        printf("Erreur lors de l'ouverture du fichier de sauvegarde.\n");
        return;
    }
    ...
    for (int i = 0; i < 3; i++)
    {
        fscanf(file, "%d %c %c %c\n", &rotors_reflector->l_r[i]->pos, rotors_reflector->l_r[i]->value, &rotors_reflector->l_r[i]->notch, &rotors_reflector->l_r[i]->turnover);
    }
    fclose(file);
}
...
void menuMachineReglable(){
    ...
    do{
        ...
        printf("2 - Sauvegarder les paramttres de la machine au moment t \n");
        ...
        printf("7 - Ouvrir la sauvegarde du parametrage de la machine \n");
        printf("==============================================\n\n");   
        if(fgets(buffer, 256, stdin) != NULL){
            ...
            switch(choix){
                ...
                case 2:
                    // Option 2 : 
                    printf("Sauvegarde des parametres de la machine\n");
                    sauvegarRotorsReflector(rotors_reflector, fopen("sauvegarde", "w"));
                    break;
                case 3:
                    // Option 3 :
                ...
                case 7:
                    // Option 7 :
                    printf("Ouverture de la sauvegarde\n");
                    chargerRotorsReflector(rotors_reflector, fopen("sauvegarde", "r"));
                    printf("Sauvegarde charger avec succes\n");
                    break;
                default:
